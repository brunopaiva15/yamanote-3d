 ...
 */
const HOLD = new THREE.Vector3(0.36, -0.36, -0.62);
/** Position portée aux lèvres : au centre, remontée, tout près. */
const DRINK = new THREE.Vector3(0.13, -0.16, -0.36);
...
export function HeldItem() {
  ...
  useFrame((_, rawDt) => {
    ...
    raise.current = Math.max(0, raise.current - dt * 1.7);
    // Une courbe en cloche : on monte vite, on tient, on redescend.
    const lift = Math.sin(Math.min(1, raise.current) * Math.PI) ** 0.7;

    if (g) {
      swing.current += dt;
      const target = HOLD.clone().lerp(DRINK, lift);
      ...
      // Inclinaison : posée dans la main au repos, franchement basculée quand
      // on boit - une bouteille ne se vide pas à la verticale.
      g.rotateX(-0.25 - lift * 1.05);
      g.rotateZ(0.18 - lift * 0.12);
    }
  ...
  return (
    <>
      <group ref={group} visible={!!product}>
        {product && <Vessel product={product} held={held} />}
      </group>
