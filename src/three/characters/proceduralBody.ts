// Le corps d'un voyageur, construit une fois pour toutes à partir de son
// apparence.
//
// Ce fichier est un DÉPLACEMENT, pas une écriture : tout ce qu'il contient
// vivait dans `three/ProceduralPlatformCrowd`, où il servait à la foule du quai
// et à elle seule. Les joueurs distants d'un salon ont besoin exactement du
// même corps - même silhouette, mêmes habits, même visage, même téléphone à la
// main -, et la seule alternative aurait été d'en écrire un troisième après
// celui du wagon et celui du quai.
//
// Rien n'a changé au passage, et c'est la contrainte qu'on s'est donnée : la
// foule doit rendre exactement la même chose après. Seuls trois symboles
// sortent - la hauteur de la dalle, le constructeur, et la pose de bras -, le
// reste est de la cuisine interne (caches de matériaux, géométries partagées)
// qui n'a aucune raison de circuler.
...
import { markOwned } from './library';

export const PLATFORM_Y = -0.06;
const HEAD_Y = 1.34;
...
export function buildPerson(app: Appearance, identity: number): THREE.Group {
...
export function crowdArmTarget(action: PaxAction, t: number): [number, number] | null {
