...
/**
 * Assise manuelle : plier quelqu'un en position assise, os par os.
 *
 * Le repli quand le pack de modèles n'a pas de clip assis - c'est le cas de
 * celui qui est versionné, dont les onze variantes ne connaissent qu'un
 * `Idle_Neutral`. Sans lui, un personnage « assis » reste debout, planté dans
 * la banquette : le défaut se voyait sur les joueurs distants, qui ne
 * passaient pas par ici.
 *
 * Sorti de `applyPoseOverrides` sans rien changer d'autre que ses entrées : il
 * ne lui fallait du passager que son cap et sa taille, et les avatars du salon
 * ont l'un comme l'autre.
 *
 * @param active vrai tant qu'il faut tenir l'assise ; le poids monte et
 *   redescend tout seul, si bien qu'on peut l'appeler à chaque image.
 * @param yaw cap du CORPS (et non du regard) : c'est lui qui décide de quel
 *   côté partent les cuisses.
 */
export function applySitPose(
  ...
  state.sitW = lerpW(state.sitW, active ? 1 : 0, k);
  if (state.sitW <= 0.001) return;
  {
    const bones = clone.bones;
    ...
    const shinLen = (legs?.shinLen ?? 0.35) * height;
    const ankleY = (legs?.ankleH ?? 0.05) * height;
...
export function applyPoseOverrides(
  ...
  applySitPose(clone, state, k, manualSit && seated, p.yaw, p.height);
