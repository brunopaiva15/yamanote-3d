...
export function ConcourseNetwork({
  ...
  // LE HALL EXISTE À TOUS LES PALIERS, ET DEPUIS PARTOUT.
  //
  // Il sautait entièrement à « très basse » : le joueur qui descendait s'y
  // retrouvait debout dans le vide — la marche ne connaît pas le palier de
  // qualité (règle de la phase 25), donc le sol était là et rien ne le
  // dessinait. On a d'abord gardé l'économie depuis le QUAI, en se disant que
  // le hall n'y était que du fond de champ.
  //
  // Il ne l'est pas. Treize gares portent leur hall AU-DESSUS des voies : la
  // volée montante y débouche en plein ciel, et l'on voyait la ville par le
  // plafond du hall qu'on s'apprête à parcourir. Ce n'est pas du fond de champ,
  // c'est le bâtiment. L'économie du palier reste entière — elle se fait sur ce
  // qu'il y a DEDANS (`Fixtures` rend les volumes sans leur décoration dès le
  // palier 2), pas sur le fait qu'il soit là.
