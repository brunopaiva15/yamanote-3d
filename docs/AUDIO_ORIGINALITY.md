...
The two main-loop clips (`01_…_inner-main`, `02_…_outer-main`) were re-cut as one singing voice over an accompaniment — a glass bell carrying the right hand at its written pitches, a bright acoustic piano playing the left hand beneath it — and do carry full metadata (`legacyGeneratedAsset: false`, seed, tempo 110, 4/4, tonal centre C). They are two interpretations of a single score held in `scripts/piano-melody-gen.py`: pitches, octaves, note lengths, rests, and rhythmic positions are shared by construction, and only touch, velocity, voice balance, pedal, stereo placement, echo, and colour differ between the Inner and Outer readings. Neither voice adds a note: the bell reads `SCORE_RIGHT` and the piano `SCORE_LEFT`, verbatim. The bell uses the same partial ratios as `synthbell` in `melodies-gen.py` — plus one octave partial folded into the voice for shimmer — which is what puts these two clips in the same sound family as the other seventeen. The score is newly written for this project — no transcription, no quoted motif, no reference to any existing jingle.
