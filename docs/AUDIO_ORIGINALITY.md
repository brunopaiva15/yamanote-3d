...
The two main-loop clips (`01_…_inner-main`, `02_…_outer-main`) were re-cut for bright acoustic piano — with the right hand shadowed an octave up by a celesta-like voice some 18 dB below it, and a sympathetic-resonance halo from the undamped strings — and do carry full metadata (`legacyGeneratedAsset: false`, seed, tempo 110, 4/4, tonal centre C). They are two interpretations of a single score held in `scripts/piano-melody-gen.py`: pitches, octaves, note lengths, rests, and rhythmic positions are shared by construction, and only touch, velocity, hand balance, pedal, stereo placement, and piano colour differ between the Inner and Outer readings. The octave doubling adds no note either: `score_double()` derives it from `SCORE_RIGHT` by transposing exactly twelve semitones. The score is newly written for this project — no transcription, no quoted motif, no reference to any existing jingle.
