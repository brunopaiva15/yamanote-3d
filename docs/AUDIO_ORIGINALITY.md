...
The two main-loop clips (`01_…_inner-main`, `02_…_outer-main`) were re-cut for bright acoustic piano, both hands, with a glass sparkle sitting 25 dB under the right-hand attack — and do carry full metadata (`legacyGeneratedAsset: false`, seed, tempo 110, 4/4, tonal centre C). They are two interpretations of a single score held in `scripts/piano-melody-gen.py`: pitches, octaves, note lengths, rests, and rhythmic positions are shared by construction, and only touch, velocity, voice balance, pedal, stereo placement, echo, and colour differ between the Inner and Outer readings. Nothing adds a note: the piano reads `SCORE_RIGHT` and `SCORE_LEFT` verbatim, and the sparkle reads `SCORE_RIGHT` too. That sparkle uses the same partial ratios as `synthbell` in `melodies-gen.py`, which keeps a thread to the other seventeen clips without becoming a second voice. The score is newly written for this project — no transcription, no quoted motif, no reference to any existing jingle.
